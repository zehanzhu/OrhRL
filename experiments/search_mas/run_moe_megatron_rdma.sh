...
DEFAULT_CONFIG_NAME="train_moe_megatron_rdma"
...
DEFAULT_LOG_PATH="$REPO_ROOT/outputs/logs/search_mas_megatron_moe_rdma_${TIMESTAMP}.log"
...
# Minimal RDMA/NCCL defaults. The RDMA launcher intentionally resets stale
# values for these keys. Set ORCHRL_KEEP_NCCL_ENV=1 to preserve them.
if [[ "${ORCHRL_KEEP_NCCL_ENV:-0}" != "1" ]]; then
...
echo "[INFO] NCCL_IB_DISABLE: ${NCCL_IB_DISABLE:-<unset>}"
echo "[INFO] NCCL_NET_GDR_LEVEL: ${NCCL_NET_GDR_LEVEL:-<unset>}"
