...
class TrainingRequirementsDocsTests(unittest.TestCase):
    def test_root_requirements_and_readme_document_training_setup(self):
        ...
        self.assertIn('match_mode', readme_text)
        self.assertIn('separate conda environment', readme_text.lower())
        self.assertIn('mas_log_dir', readme_text)
        self.assertIn('stdout/stderr', readme_text.lower())
        ...
        self.assertIn('match_mode', training_flow_text)
        self.assertIn('failure_policy', training_flow_text)
        self.assertIn('validation/failed_sample_rate', training_flow_text)
        self.assertIn('training/rollout_failed_job_rate', training_flow_text)
        self.assertIn('mas_log_dir', training_flow_text)
        ...
        self.assertIn('train_repeat', experiment_readme_text)
        self.assertIn('train_shuffle', experiment_readme_text)
        self.assertIn('match_mode', experiment_readme_text)
        self.assertIn('validation/failed_sample_rate', experiment_readme_text)
        self.assertIn('mas_log_dir', experiment_readme_text)
        ...
        self.assertIn('ORCHRL_TRANSFORMER_ENGINE_HOME', experiment_readme_text)
        self.assertIn('transformer-engine-cu128-torch290', experiment_readme_text)
        self.assertIn('role_specific', readme_text)
        self.assertIn('role_sharing', readme_text)
        self.assertIn('cfg.training.train_data_path', experiment_launcher_text)
        self.assertIn('cfg.training.val_data_path', experiment_launcher_text)
        self.assertIn('ORCHRL_TRANSFORMER_ENGINE_HOME', experiment_launcher_text)
        self.assertIn('LD_LIBRARY_PATH', experiment_launcher_text)
        self.assertIn('for idx, policy_cfg in enumerate(cfg.base_models.values())', experiment_launcher_text)
        self.assertIn('print(f"MODEL_PATH_{idx}={shlex.quote(str(policy_cfg.path))}")', experiment_launcher_text)
        self.assertNotIn('cfg.base_models.policy_1.path', experiment_launcher_text)
        self.assertNotIn('cfg.base_models.policy_2.path', experiment_launcher_text)
        self.assertNotIn('cfg.training.mate.prompt_loader.path', experiment_launcher_text)
